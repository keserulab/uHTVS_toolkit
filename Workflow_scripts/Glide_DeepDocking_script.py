...
# The grid.zip file for the protein has to be created by Glide (receptor grid generation)
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
# The following section of inputs has to be set manually. The first series of inputs sets the parameters of the DeepDocking workflow, the second series of inputs handles the GlideHTVS workflow

# The required variables for DD scripts

dd_loc = ""  								                        	# Location of the deepdocking python scripts
project_name = ""                                                         				# Name of the project folder (this has to be made manually)
morgan_loc = ""                  									# Location of the morgan fingerprints
smiles_loc = ""                  									# Location of the splitted smiles files (they are in .txt format)
num_cpus =                                                                                    		# Number of cpus to be used for the scripts:
sampled_num =                                                                            		# Number of sampled molecules for each iterations (for the first iteration, this will be multiplied by 3)
...
mol_percent_first =                                                                         		# Percentage of the molecules labeled as hits at the first iterations
mol_percent_last =                                                                        		# Percentage of the molecules labeled as hits at the last iterations
num_mol_exported =                                                                         		# Number of molecules to be exported at the end of the workflow

# The required variables for GlideHTVS Workflow

glide_workflow_script_loc = ""                             						# Location of the GlideHTVS workflow scripts
glide_grid = ""                    									# Location of the Glide grid file (filename.zip), including file name
cpus =                                                                                        		# Number of CPUs to use for the docking process 
sj =                                                                                          		# Number of subjobs in the docking process
proton = ''                                                                                  		# Find the major protonation form at pH: 7.4? yes/no
conf = ''                                                                                    		# Perform local optimization on generated 3D structure? yes/no
remove = ''                                                                                  		# Delete ligand pdbqt, dlg, xml and Batch files? yes/no
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------

#the next values are default values for DD according to the authors, but can be changed here if necessary:

num_it = 11                                                                         			# Number of iterations in integer (default is 11)
...
txt6_test = "python {}Glide_WorkFlow_smiles2score_main.py -i {}{}/iteration_{}/smile/test_smiles_final_updated.smi -f {} -o {}{}/iteration_{}/testing_labels_pre.txt -grid {} -nc {} -sj {} -prot {} -loc {} -d {}"
txt6_train = "python {}Glide_WorkFlow_smiles2score_main.py -i {}{}/iteration_{}/smile/train_smiles_final_updated.smi -f {} -o {}{}/iteration_{}/training_labels_pre.txt -grid {} -nc {} -sj {} -prot {} -loc {} -d {}"
txt6_valid = "python {}Glide_WorkFlow_smiles2score_main.py -i {}{}/iteration_{}/smile/valid_smiles_final_updated.smi -f {} -o {}{}/iteration_{}/validation_labels_pre.txt -grid {} -nc {} -sj {} -prot {} -loc {} -d {}"
...
for i in range(starting_it, num_it+1):
    ...
    if cont == 0:
      ...
      # Usage of the GlideHTVS workflow

      out = subprocess.check_output(txt6_test.format(glide_workflow_script_loc, project_folder_loc, project_name,  i, glide_workflow_script_loc, project_folder_loc, project_name, i, glide_grid, cpus, sj, proton, conf, remove), shell=True, universal_newlines=True)
      out = subprocess.check_output(txt6_train.format(glide_workflow_script_loc, project_folder_loc, project_name, i, glide_workflow_script_loc, project_folder_loc, project_name, i, glide_grid, cpus, sj, proton, conf, remove), shell=True, universal_newlines=True)
      out = subprocess.check_output(txt6_valid.format(glide_workflow_script_loc, project_folder_loc, project_name, i, glide_workflow_script_loc, project_folder_loc, project_name, i, glide_grid, cpus, sj, proton, conf, remove), shell=True, universal_newlines=True)
